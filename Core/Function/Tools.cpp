...
LinInterpol get_integral(const Func1D &f, unsigned int steps) {
    ...
    vector<double> F_start = {f.limits().get_lower(), 0.0};
    F.push_back(F_start);
    for (unsigned int i = 1; i < steps; i++) {
        double x = f.limits().get_lower() + i*step;
    ...
    double x_end_in_f = nextafter(x_end, f.limits().get_lower());
    vector<double> F_end = {
        x_end,
