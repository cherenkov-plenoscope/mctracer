...
void MmcsCorsikaPhotonData::append_valid_photons_from_sub_block(
	...
	for(unsigned int row=0; row<sub_block.rows(); row++) {
		
		std::vector<float> photon_row;
		for(unsigned int col=0; col<sub_block.cols(); col++) {

			const unsigned int sub_block_position = row*sub_block.cols()+col;
...
//------------------------------------------------------------------------------
unsigned int MmcsCorsikaPhotonData::number_of_photons()const {
	return photon_matrix.size();
}
//------------------------------------------------------------------------------
void MmcsCorsikaPhotonData::access_is_in_range(const unsigned int at)const {
...
//------------------------------------------------------------------------------
int MmcsCorsikaPhotonData::get_parent_code(const unsigned int at)const {
	access_is_in_range(at);
	return get_code_holder(at)/100000;
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_x_pos_on_world_x_y_plane_in_cm(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(1);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_y_pos_on_world_x_y_plane_in_cm(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(2);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_cosine_between_dir_and_world_x_axis(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(3);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_cosine_between_dir_and_world_y_axis(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(4);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_wavelength_in_nm(const unsigned int at)const {
	access_is_in_range(at);
	return fmod(photon_matrix.at(at).at(0),1000.0);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_production_time_in_ns(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(5);
}
//------------------------------------------------------------------------------
float MmcsCorsikaPhotonData::get_production_height_in_cm(const unsigned int at)const {
	access_is_in_range(at);
	return photon_matrix.at(at).at(6);
}
//------------------------------------------------------------------------------
int MmcsCorsikaPhotonData::get_code_holder(const unsigned int at)const {
	access_is_in_range(at);
	return int(round(photon_matrix.at(at).at(0)));
}
//------------------------------------------------------------------------------
std::string MmcsCorsikaPhotonData::get_photon_print_at(const unsigned int at)const {
	std::stringstream out;
