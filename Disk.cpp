...
//==================================================================
bool Disk::set_disk(double dbl_new_disk_radius_in_m){
	//===================
	// set disk radius
	//===================
	if(dbl_new_disk_radius_in_m > 0.0){
		dbl_disk_radius_in_m = dbl_new_disk_radius_in_m;
	}else{
		std::cout<<"set_disk -> disk_radius: ";
		std::cout<<dbl_new_disk_radius_in_m<<" <= 0.0";
		std::cout<<" is not valid!"<<std::endl;
		return false;
	}
	// calculate max radius of max_norm
	radius_of_sphere_enclosing_all_children = dbl_disk_radius_in_m;
...
void Disk::disp(){
	...
	out.str("");
	out<<"disk:"<<name_of_frame<<"_________________________________"<<std::endl;
...
//==================================================================
std::string Disk::get_disk_string(){
	std::stringstream out;
	out.str("");
	out<<"||| disk_radius: "<<dbl_disk_radius_in_m<<" [m]"<<std::endl;
	return out.str();
}
//==================================================================
void Disk::hit(Vector3D *base,Vector3D *dir, Intersection *intersection)const{
	// hit_flag = false;
	...
	if(dir->get_z() == 0.0)
	{
		// the disk-surface is parallel to the ray
		...
		// the ray is not parallel to the disk-surface
		//double v;
