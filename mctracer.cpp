...
int main(int argc, char* argv[]) {
	try{
		...
		//std::cout << Intersection::source_object->get_print();
		...
		*/
		...
		//ground
		Disc ground;
		ground.set_frame("ground", Vector3D(0.0, 0.0, -2.0), Rotation3D(0.0,0.0,0.0));
 		
		ColourProperties* ground_color = new ColourProperties(0,55,0);
		...
		//telescope
		TelescopeFrame FACT;
		FACT.set_frame("FACT", Vector3D(0.0, 0.0, 0.0), Rotation3D(0.0,0.0,0.0));

		//reflector
		FactTelescope Reflector(0.5);
		...
		sensor_housing_lower_cap.set_frame("sensor_housing_lower_cap", Vector3D(0.0, 0.0, 4.890), Rotation3D(0.0,0.0,0.0));
		ColourProperties* sensor_housing_color = new ColourProperties(200,255,200);
		...
		Mworld.set_mother_and_child(&FACT);
		Mworld.set_mother_and_child(&ground);
		Mworld.setup_tree_based_on_mother_child_relations();
		...
		while(event_getter.has_still_events_left()) {
			...
			while(event.can_be_reused_again()) {
				...
				FACT.move_to_Az_Zd_in_Rad(event.get_Az(), event.get_Zd());
