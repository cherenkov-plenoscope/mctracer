...
TEST_CASE("camera_operatorTest: increase_FoV", "[merlict]") {
    ...
    CHECK(ct.cam.get_FoV_in_rad() > ml::deg2rad(160.0));
    CHECK(ct.cam.get_FoV_in_rad() < ml::deg2rad(180.0));
}
...
TEST_CASE("camera_operatorTest: decrease_FoV", "[merlict]") {
    ...
    CHECK(ct.cam.get_FoV_in_rad() > ml::deg2rad(0.0));
    CHECK(ct.cam.get_FoV_in_rad() < ml::deg2rad(0.003));
}
