...
void CameraManForRotation::look_further_up_when_possible(){
	if(is_not_falling_backwards_over_when_looking_up()){
		look_further_up();
		print_camera_rotation_for_y_axis_manipulation();
	}else{
		print_camera_can_not_look_further("up");
	}
}
...
void CameraManForRotation::look_further_down_when_possible(){	
	if(is_not_falling_forward_over_when_looking_down()){
		...
		print_camera_can_not_look_further("down");
	}
}
...
void CameraManForRotation::print_camera_rotation_for_y_axis_manipulation()const{
	std::cout << camera->get_name() << " rot y: ";
	std::cout << Rad2Deg(RotWorld2CameraY_in_rad) << "DEG\n";
}

void CameraManForRotation::print_camera_can_not_look_further(
...
void CameraManForRotation::look_left(){
	increase_rot_z();
	update_R_World2Camera();
	print_camera_rotation_for_z_axis_manipulation();
}
...
void CameraManForRotation::print_camera_rotation_for_z_axis_manipulation()const{
