...
//======================
void OpticalMirrorParabolicRound::disp(){
	std::stringstream out;
	out.str("");
	out<<"parabolic round mirror:"<<name_of_frame;
...
//======================
std::string OpticalMirrorParabolicRound::get_parabolic_round(){
	std::stringstream out;
	out.str("");
...
//======================
void OpticalMirrorParabolicRound::hit(Vector3D *base,Vector3D *dir, Intersection *intersection)const{
	// hit_flag = false;
	//
	...
	if(dbl_inner_part_of_square_root_p_q < 0){
		...
		if(test_zylinder_fit(&intersec_parabola_tp))
		{
			// intersection inside defined volume
			hit_valid_tp = true;
		}
		if(test_zylinder_fit(&intersec_parabola_tm))
		{
			// intersection inside defined volume
			hit_valid_tm = true;
		}
