...
TEST_F(SetUpScenery, create_scenery) {
    ...
    tree->set_name_pos_rot("My_Tree", tree_pos, ROT3_UNITY);

    Color leaf_green(0, 128, 0);
    Sphere* leaf_ball = tree->append<Sphere>();
    leaf_ball->set_name_pos_rot("leaf_ball", Vec3(0.0, 0.0, 2.0), ROT3_UNITY);
    ...
    tree_pole->set_name_pos_rot("tree_pole", Vec3(0.0, 0.0, 0.5), ROT3_UNITY);
    ...
    house->set_name_pos_rot("house", house_pos, ROT3_UNITY);
    ...
    foundation->set_name_pos_rot("foundation", Vec3(0.0, 0.0, 0.01), ROT3_UNITY);
    ...
    front_wall->set_name_pos_rot("wall1", Vec3(0.0, 0.0, 0.0), ROT3_UNITY);
