...
TimeAndCount get_arrival_time_and_count_given_arrival_moments_and_integration_time_window(
    ...
    vector<unsigned int> enclo =
        convolution_of_arrival_times_and_integration_window(
            arrival_moments,
...
vector<int> simulation_truth_ids_in_time_window(
    ...
    for (
        unsigned int idx = start_pulse_index;
        idx < arrival_moments.size();
        idx++
    ) {
        if (
            arrival_moments[idx].arrival_time - start_time >
            integration_time_window
        ) {
            break;
        } else {
            simulation_truth_ids.push_back(
                arrival_moments[idx].simulation_truth_id);
        }
    }
...
void write_intensity_simulation_truth(
    ...
    if (file.is_open()) {
        for (unsigned int i = 0; i < tacs.size(); i++) {
            for (
                unsigned int j = 0;
                j < tacs[i].simulation_truth_ids.size();
                j++
            ) {
