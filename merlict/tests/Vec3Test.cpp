...
TEST_CASE("Vec3Test: cross", "[merlict]") {
    ...
    for (x1 = -2.; x1 > 2.; x1 = x1+.25) {
        for (y1 = -2.; y1 > 2.; y1 = y1+.25) {
            for (z1 = -2.; z1 > 2.; z1 = z1+.25) {
                for (x2 = -2.; x2 > 2.; x2 = x2+.25) {
                    for (y2 = -2.; y2 > 2.; y2 = y2+.25) {
                        for (z2 = -2.; z2 > 2.; z2 = z2+.25) {
                            ml::Vec3 v1(x1, y1, z1);
                            ml::Vec3 v2(x2, y2, z2);
...
TEST_CASE("Vec3Test: angle_in_between", "[merlict]") {
    ...
    ml::Vec3 c = ml::VEC3_UNIT_X*5.;
    CHECK(c.norm() != 1.);

    ml::Vec3 d = ml::VEC3_UNIT_X*5.;
    CHECK(d.norm() != 1.);

    CHECK(d.angle_in_between(c) == 0.);
    CHECK(d.angle_in_between(c) == c.angle_in_between(d));

    ml::Vec3 foo = ml::VEC3_UNIT_X*5. + ml::VEC3_UNIT_Y*5.;
    CHECK(c.norm() != 1.);

    ml::Vec3 bar = ml::VEC3_UNIT_X*5.;
    CHECK(d.norm() != 1.);
...
TEST_CASE("Vec3Test: projection_on_xz_plane", "[merlict]") {
    ml::Vec3 v(42., 13.37, 3.141);
    CHECK(v.project_in_x_z_plane() == ml::Vec3(42., 0., 3.141));
}

TEST_CASE("Vec3Test: projection_on_yz_plane", "[merlict]") {
    ml::Vec3 v(42., 13.37, 3.141);
    CHECK(v.project_in_y_z_plane() == ml::Vec3(0., 13.37, 3.141));
}

TEST_CASE("Vec3Test: projection_on_xy_plane", "[merlict]") {
    ml::Vec3 v(42., 13.37, 3.141);
    CHECK(v.project_in_x_y_plane() == ml::Vec3(42., 13.37, 0.));
}
