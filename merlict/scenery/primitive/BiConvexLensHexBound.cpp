...
void BiConvexLensHexBound::set_curvature_radius_and_outer_hex_radius(
    ...
    std::shared_ptr<SphereCapWithHexagonalBound> front_cap =
        add<SphereCapWithHexagonalBound>();
    std::shared_ptr<SphereCapWithHexagonalBound> rear_cap =
        add<SphereCapWithHexagonalBound>();
    ...
    std::vector<std::shared_ptr<PlaneDualSphericalBound>> walls(6);
