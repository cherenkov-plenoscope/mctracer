...
void RectangularBox::set_xyz_width(
    ...
    walls[0]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));

    walls[1]->set_name_pos_rot(
        get_name()+"_wall_1", z_off*-1.0, Rot3(M_PI, 0.0, 0.0));
    walls[1]->set_x_y_width(x_width, y_width);
    walls[1]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));
    ...
    walls[2]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));

    walls[3]->set_name_pos_rot(
        get_name()+"_wall_3", x_off*-1.0, Rot3(0.0, M_PI/2.0, 0.0));
    walls[3]->set_x_y_width(z_width, y_width);
    walls[3]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));
    ...
    walls[4]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));

    walls[5]->set_name_pos_rot(
        get_name()+"_wall_5", y_off*-1.0, Rot3(-M_PI/2.0, 0.0, 0.0));
    walls[5]->set_x_y_width(x_width, z_width);
    walls[5]->adopt_surface(std::static_pointer_cast<const SurfaceEntity>(this->shared_from_this()));
}
